...
//           `http://localhost:3000/v1/post?page=${page}&limit=2&category=Wrting`
//         );
...
const Khmer = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/v1/post?page=${page}&limit=2&category=khmer`
        );
  ...
  return (
    <div>
      <div className="flex ms-3 w-auto h-auto mt-28">
        ...
        <div className="items-center flex-row">
          <Typography fontSize="header" color="categories">Khmer</Typography>
        </div>
      </div>
      <div className="flex flex-col gap-2 mt-2">
        {posts.length === 0 ? (
