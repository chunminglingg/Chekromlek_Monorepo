...
const GeneralKnowledge = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch(`http://localhost:3000/v1/post?page=${page}&limit=2&category=General Knowledge`);
        if (!response.ok) {
    ...
    <div>
      <div className="flex ms-3 w-auto h-auto">
        ...
        <div className="items-center w-[420px] flex-row">
          <p className="text-purple-600 font-bold text-[23px]">General Knowledge</p>
        </div>
      </div>
