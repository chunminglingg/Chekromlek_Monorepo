...
//           `http://localhost:3000/v1/post?page=${page}&limit=2&category=Physic`
//         );
...
const Physic = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/v1/post?page=${page}&limit=2&category=Physical`
        );
        const data = response.data;
  ...
  return (
    <div>
      <div className="flex ms-3 w-auto h-auto mt-28">
        ...
        <div className="items-center flex-row">
          <Typography fontSize="header" color="categories">Physical</Typography>
        </div>
      </div>
      <div className="flex flex-col gap-2 mt-2">
        {posts.length === 0 ? (
